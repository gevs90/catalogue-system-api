    ...
    def authenticate(
        ...
        if user.password is None:
            ...
            if not self.verify(user.password, login.password):
                raise_with_log(status.HTTP_401_UNAUTHORIZED,
                               "Incorrect password")
            else:
...
class AuthDataManager(BaseDataManager):
    ...
    def get_emails_users(self, user: any) -> list[EmailSchema]:
        """Get emails of admins."""

        users = self.get_all(
            select(UserModel).where(UserModel.role == "admin").where(UserModel.email != user.email))
        return users
